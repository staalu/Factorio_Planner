...
class 量化计算:
    ...
    def 特殊产品处理(self):
        ...
        if 石化开关:
            self.石化原料, self.石化配方设备数量 = self.石化量化计算()

            等待删除 = []
            for 某物,秒量 in self.石化原料.items():
                if abs(秒量) < 十进制(0.000000000000000000000001):
                    等待删除.append(某物)
            for i in 等待删除:
                del self.石化原料[i]
    ...
    def 石化量化计算(self):
        ...
        #初始.特殊配方选择["石油化工路径"]
        石化配方 = copy.deepcopy(初始.配方[初始.特殊配方选择["石油化工路径"]])
        重油裂解配方 = copy.deepcopy(初始.配方["重油裂解"])
        轻油裂解配方 = copy.deepcopy(初始.配方["轻油裂解"])
        石化目标 = copy.deepcopy(self.石化量化目标)
        石化配方设备数量 = {初始.特殊配方选择["石油化工路径"]:十进制(0), "重油裂解":十进制(0), "轻油裂解":十进制(0) }


        重油化工配方 = [石化配方, { 初始.特殊配方选择["石油化工路径"]:十进制(1), "重油裂解":十进制(0), "轻油裂解":十进制(0) } ]
        #print(重油化工配方)
        
        轻油化工配方 = copy.deepcopy(重油化工配方)
        每石化配方全重油裂解数量 = 轻油化工配方[0]["重油"] / 重油裂解配方["重油"]
        for 某物,秒量 in 重油裂解配方.items():
            if 某物 in 轻油化工配方[0]:
                轻油化工配方[0][某物] = 轻油化工配方[0][某物] - ( 重油裂解配方[某物] * 每石化配方全重油裂解数量 )
            else:
                轻油化工配方[0][某物] = - ( 重油裂解配方[某物] * 每石化配方全重油裂解数量 )
        轻油化工配方[1]["重油裂解"] = abs(每石化配方全重油裂解数量)
        del 轻油化工配方[0]["重油"]
        #print(轻油化工配方)

        石油气化工配方 = copy.deepcopy(轻油化工配方)
        每石化配方全轻油裂解数量 = 石油气化工配方[0]["轻油"] / 轻油裂解配方["轻油"]
        for 某物,秒量 in 轻油裂解配方.items():
            if 某物 in 石油气化工配方[0]:
                石油气化工配方[0][某物] = 石油气化工配方[0][某物] - ( 轻油裂解配方[某物] * 每石化配方全轻油裂解数量 )
            else:
                石油气化工配方[0][某物] = - ( 轻油裂解配方[某物] * 每石化配方全轻油裂解数量 )
        石油气化工配方[1]["轻油裂解"] = abs(每石化配方全轻油裂解数量)
        del 石油气化工配方[0]["轻油"]
        #print(石油气化工配方)
        ...
        某油化工配方 = {"重油":重油化工配方 , "轻油":轻油化工配方 , "石油气":石油气化工配方 , }
        某油化工配方数量 = { "重油":十进制(0), "轻油":十进制(0), "石油气":十进制(0), }

        #print(石化目标)
        for 某油 in ["重油", "轻油", "石油气"]:
            if 石化目标[某油] > 十进制(0):
                某油化工配方数量[某油] = 石化目标[某油] / 某油化工配方[某油][0][某油]
                石化配方设备数量[初始.特殊配方选择["石油化工路径"]] += 某油化工配方数量[某油]
                石化配方设备数量["重油裂解"] += 某油化工配方数量[某油] * 某油化工配方[某油][1]["重油裂解"]
                石化配方设备数量["轻油裂解"] += 某油化工配方数量[某油] * 某油化工配方[某油][1]["轻油裂解"]
                for 某设备,数量 in 石化配方设备数量.items():
                    if abs(数量) < 十进制(0.000000000000000000000001):
                        石化配方设备数量[某设备] = 十进制(0)
                for 某物,秒量 in 某油化工配方[某油][0].items():
                    if 某物 in 石化目标:
                        石化目标[某物] += -(某油化工配方数量[某油] * 某油化工配方[某油][0][某物])
                    else:
                        石化目标[某物]  = -(某油化工配方数量[某油] * 某油化工配方[某油][0][某物])
                if abs(石化目标[某油]) < 十进制(0.000000000000000000000001):
                    石化目标[某油] = 十进制(0)
            #print(石化目标)
    ...
    def 显示内容(self):
        ...
        if self.石化配方设备数量:
            #print(self.石化量化目标)
            print("\n另外 每秒需要消耗以下数量的石化产品:")
            for 某物,秒量 in self.石化量化目标.items():
                print("每秒钟会有 {0:.3f} 单位的 {1} 被消耗".format( 秒量 , 某物 ))

            #print(self.石化配方设备数量)
            print("\n需要以下设备进行生产:")
            for 配方,设备数量 in self.石化配方设备数量.items():
                print("需要 {0:.3f} 个 {1} 生产 {2}".format(设备数量, 初始.配方对应设备[配方], 配方))
            ...
            for 某物,秒量 in self.石化原料.items():
                if 秒量 > 十进制(0):
                    ...
                    print("每秒钟会有 {0:.3f} 单位的 {1} 生成却无处消耗".format( abs(秒量) , 某物 ))
