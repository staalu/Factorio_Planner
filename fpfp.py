        ...
        try:
            for 行 in 基本数据行:
                if 行[0] == "!" and len(行) > 1 :
                    ...
                    locals()[当前类别].append(行.strip())
                elif 行.strip() and 当前类别 == "产能加成":
                    分割行 = 行.split("=")
                    locals()[当前类别][分割行[0].strip()] = 十进制(分割行[1].strip())
                elif 行.strip() and 当前类别 == "速度加成":
                    分割行 = 行.split("=")
                    locals()[当前类别][分割行[0].strip()] = 十进制(分割行[1].strip())

            半条传送带运力 = []
            for 传送带类型 in 传送带运力.keys():
                半条传送带运力.append(["半条" + 传送带类型 ,传送带运力[传送带类型] / 十进制("2")])
...
class 单位时间配方条目类:
    ...
    def __init__( self, 数据源 , 设备速度 ):
        ...
        self.产品 = 数据源[1][0]
        try:
            产品产能加成 = 初始.产能加成[self.产品]
        except:
            产品产能加成 = 十进制(0)
        try:
            设备速度加成 = 初始.速度加成[self.产品]
        except:
            设备速度加成 = 十进制(0)
            pass

        self.产量 = self.计算每秒量( 数据源[1][1]*(十进制(1)+产品产能加成), 配方时间, 特定设备速度*(十进制(1)+设备速度加成) )
        材料字典 = 数据源[2]

        for 材料名 in 材料字典:
            配方材料消耗 = 材料字典[材料名]
            每秒耗材量 = self.计算每秒量(配方材料消耗, 配方时间, 特定设备速度*(十进制(1)+设备速度加成))
            材料字典[材料名] = 每秒耗材量
    ...
    while True:
        ...
        for (产物名称,产物数量) in 规划.中间产物数量.items():
            ...
            for (传送带类型,此类型的速度) in 初始.传送带运力:
                if 产物数量 > 此类型的速度:
                    超运力字段 = "    这已经超过 {0} 的运力".format(传送带类型)
            if 产物名称.split("at")[0] in 初始.管道运输流体:
                超运力字段 = ""
